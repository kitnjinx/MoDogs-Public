...
public class BasenjiEntity extends AbstractDog {
    ...
    private <E extends BasenjiEntity> PlayState predicate(AnimationEvent<E> event) {
        if (this.isSitting()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.basenji.sitting"));
            return PlayState.CONTINUE;
        }

        if (this.isAngry() || this.isAggressive() & event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.basenji.angrywalk"));
            return PlayState.CONTINUE;
        }

        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.basenji.walk"));
            return PlayState.CONTINUE;
        }

        if (this.isAngry() || this.isAggressive()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.basenji.angryidle"));
            return PlayState.CONTINUE;
        }

        event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.basenji.idle"));
        return PlayState.CONTINUE;
    }
    @Override
        ...
        if (item == ModItems.GENE_TESTER.get()) {
            if (this.level.isClientSide) {
                ...
                if (this.getVariant() == BasenjiVariant.TRICOLOR) {
                    message = new TextComponent("This Basenji demonstrates a recessive trait.");
                } else if (this.getVariant() == BasenjiVariant.BLACK) {
                    if (this.getCarrier() == 0) {
                        message = new TextComponent("This Basenji demonstrates a trait that can be recessive.");
                    } else {
                        message = new TextComponent("This Basenji demonstrates a trait that can be recessive, however they also carry the more recessive tricolor fur trait.");
                    }
                } else {
                    if (this.getCarrier() == 0) {
                        message = new TextComponent("This Basenji doesn't have any recessive traits.");
                    } else if (this.getCarrier() == 1) {
                        message = new TextComponent("This Basenji carries a recessive trait for black fur.");
                    } else {
                        message = new TextComponent("This Basenji carries a recessive trait for tricolor fur.");
                    }
                }
