...
public class BerneseMountainDogEntity extends AbstractDog {
    ...
    private <E extends BerneseMountainDogEntity> PlayState predicate(AnimationEvent<E> event) {
        if (this.isSitting()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.bernese_mountain_dog.sitting"));
        ...
        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.bernese_mountain_dog.walk"));
            return PlayState.CONTINUE;
        }

        if (this.isAngry() || this.isAggressive()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.bernese_mountain_dog.angryidle"));
            return PlayState.CONTINUE;
        }

        event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.bernese_mountain_dog.idle"));
        return PlayState.CONTINUE;
    }
    @Override
        ...
        if (item == ModItems.GENE_TESTER.get()) {
            if (this.level.isClientSide) {
                ...
                if (this.getVariant() == BerneseMountainDogVariant.TAN) {
                    message = new TextComponent("This Bernese Mountain Dog demonstrates a recessive trait.");
                } else if (this.getCarrier()) {
                    message = new TextComponent("This Bernese Mountain Dog carries a recessive trait.");
                } else {
                    message = new TextComponent("This Bernese Mountain Dog doesn't have any recessive traits.");
                }
