...
public class BostonTerrierEntity extends AbstractDog {
    ...
    private <T extends GeoAnimatable> PlayState predicate(AnimationState<T> state) {
        ...
        if (this.isAngry() || this.isAggressive() & state.isMoving()) {
            state.getController().setAnimation(RawAnimation.begin().then("animation.boston_terrier.angrywalk", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }

        if (state.isMoving()) {
            state.getController().setAnimation(RawAnimation.begin().then("animation.boston_terrier.walk", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }

        if (this.isAngry() || this.isAggressive()) {
            state.getController().setAnimation(RawAnimation.begin().then("animation.boston_terrier.angryidle", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }

        state.getController().setAnimation(RawAnimation.begin().then("animation.boston_terrier.idle", Animation.LoopType.LOOP));
        return PlayState.CONTINUE;
    }
    @Override
