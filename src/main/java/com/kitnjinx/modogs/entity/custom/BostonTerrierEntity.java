...
public class BostonTerrierEntity extends AbstractDog {
    ...
    private <E extends BostonTerrierEntity> PlayState predicate(AnimationEvent<E> event) {
        if (this.isSitting()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.boston_terrier.sitting"));
            return PlayState.CONTINUE;
        }

        if (this.isAngry() || this.isAggressive() & event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.boston_terrier.angrywalk"));
            return PlayState.CONTINUE;
        }

        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.boston_terrier.walk"));
            return PlayState.CONTINUE;
        }

        if (this.isAngry() || this.isAggressive()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.boston_terrier.angryidle"));
            return PlayState.CONTINUE;
        }

        event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.boston_terrier.idle"));
        return PlayState.CONTINUE;
    }
    @Override
        ...
        if (item == ModItems.GENE_TESTER.get()) {
            if (this.level.isClientSide) {
                ...
                if (this.getVariant() == BostonTerrierVariant.SEAL) {
                    message = new TextComponent("This Boston Terrier demonstrates a recessive trait.");
                } else if (this.isCarrier()) {
                    message = new TextComponent("This Boston Terrier carries a recessive trait.");
                } else {
                    message = new TextComponent("This Boston Terrier doesn't have any recessive traits.");
                }
    ...
    protected void defineSynchedData() {
        ...
        this.entityData.define(CARRIES_SEAL, false);
    }
