...
public class CKCharlesSpanielEntity extends AbstractDog {
    ...
    private <E extends CKCharlesSpanielEntity> PlayState predicate(AnimationEvent<E> event) {
        if (this.isSitting()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.ck_charles_spaniel.sitting"));
            return PlayState.CONTINUE;
        }

        if (this.isAngry() || this.isAggressive() & event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.ck_charles_spaniel.angrywalk"));
            return PlayState.CONTINUE;
        }

        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.ck_charles_spaniel.walk"));
            return PlayState.CONTINUE;
        }

        if (this.isAngry() || this.isAggressive()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.ck_charles_spaniel.angryidle"));
            return PlayState.CONTINUE;
        }

        event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.ck_charles_spaniel.idle"));
        return PlayState.CONTINUE;
    }
    @Override
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (item == ModItems.GENE_TESTER.get()) {
            if (this.level.isClientSide) {
                ...
                if (this.hasPoints()) {
                    if (this.hasWhite()) {
                        message = new TextComponent("This Cavalier King Charles Spaniel demonstrates a recessive trait and white markings.");
                    } else {
                        message = new TextComponent("This Cavalier King Charles Spaniel demonstrates a recessive trait.");
                    }
                } else if (this.isCarrier()) {
                    if (this.hasWhite()) {
                        message = new TextComponent("This Cavalier King Charles Spaniel carries a recessive trait and demonstrates white markings.");
                    } else {
                        message = new TextComponent("This Cavalier King Charles Spaniel carries a recessive trait.");
                    }
                } else {
                    if (this.hasWhite()) {
                        message = new TextComponent("This Cavalier King Charles Spaniel demonstrates white markings.");
                    } else {
                        message = new TextComponent("This Cavalier King Charles Spaniel doesn't have any recessive traits.");
                    }
                }
