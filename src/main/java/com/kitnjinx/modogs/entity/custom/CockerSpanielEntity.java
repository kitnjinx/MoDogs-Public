...
public class CockerSpanielEntity extends AbstractDog {
    ...
    private <E extends CockerSpanielEntity> PlayState predicate(AnimationEvent<E> event) {
        if (this.isSitting()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.cocker_spaniel.sitting"));
            return PlayState.CONTINUE;
        }

        if (this.isAngry() || this.isAggressive() & event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.cocker_spaniel.angrywalk"));
            return PlayState.CONTINUE;
        }

        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.cocker_spaniel.walk"));
            return PlayState.CONTINUE;
        }

        if (this.isAngry() || this.isAggressive()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.cocker_spaniel.angryidle"));
            return PlayState.CONTINUE;
        }

        event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.cocker_spaniel.idle"));
        return PlayState.CONTINUE;
    }
    @Override
        ...
        if (item == ModItems.GENE_TESTER.get()) {
            if (this.level.isClientSide) {
                ...
                if (this.getVariant() == CockerSpanielVariant.BUFF || this.getVariant() == CockerSpanielVariant.SILVER) {
                    if (this.isBlack()) {
                        message = new TextComponent("This Cocker Spaniel demonstrates two recessive traits.");
                    } else if (this.carriesBlack()) {
                        message = new TextComponent("This Cocker Spaniel demonstrates the dilution trait and carries the recessive black fur trait.");
                    } else {
                        message = new TextComponent("This Cocker Spaniel demonstrates the dilution trait.");
                    }
                } else if (this.carriesDilute()) {
                    if (this.isBlack()) {
                        message= new TextComponent("This Cocker Spaniel demonstrates the black fur trait and carries the recessive dilution trait.");
                    } else if (this.carriesBlack() && this.isBrown()) {
                        message = new TextComponent("This Cocker Spaniel demonstrates a rare variant of red fur. They also carry two recessive traits.");
                    } else if (this.isBrown()) {
                        message = new TextComponent("This Cocker Spaniel demonstrates a rare variant of red fur and carries the dilution trait.");
                    } else if (this.carriesBlack()) {
                        message = new TextComponent("This Cocker Spaniel carries two recessive traits.");
                    } else {
                        message = new TextComponent("This Cocker Spaniel carries the recessive dilution trait.");
                    }
                } else {
                    if (this.isBlack()) {
                        message = new TextComponent("This Cocker Spaniel demonstrates the recessive black fur trait.");
                    } else if (this.carriesBlack() && this.isBrown()) {
                        message = new TextComponent("This Cocker Spaniel demonstrates a rare variant of red fur and carries the recessive black fur trait.");
                    } else if (this.carriesBlack()) {
                        message = new TextComponent("This Cocker Spaniel carries the recessive black fur trait.");
                    } else if (this.isBrown()) {
                        message = new TextComponent("This Cocker Spaniel demonstrates a rare variant of red fur.");
                    } else {
                        message = new TextComponent("This Cocker Spaniel doesn't have any recessive traits.");
                    }
                }
