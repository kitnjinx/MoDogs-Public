...
public class DobermanEntity extends AbstractDog {
    ...
    private <E extends DobermanEntity> PlayState predicate(AnimationEvent<E> event) {
        if (this.isSitting()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.doberman.sitting"));
            return PlayState.CONTINUE;
        }

        if (this.isAngry() || this.isAggressive() & event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.doberman.angrywalk"));
            return PlayState.CONTINUE;
        }

        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.doberman.walk"));
            return PlayState.CONTINUE;
        }

        if (this.isAngry() || this.isAggressive()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.doberman.angryidle"));
            return PlayState.CONTINUE;
        }

        event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.doberman.idle"));
        return PlayState.CONTINUE;
    }
    @Override
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (item == ModItems.GENE_TESTER.get()) {
            if (this.level.isClientSide) {
                ...
                if (this.isRed()) {
                    if (this.isDilute()) {
                        message = new TextComponent("This Doberman demonstrates two recessive traits.");
                    } else if (this.carriesDilute()) {
                        message = new TextComponent("This Doberman demonstrates the recessive red fur trait and carries the dilution trait.");
                    } else {
                        message = new TextComponent("This Doberman demonstrates the recessive red fur trait.");
                    }
                } else if (this.carriesRed()) {
                    if (this.isDilute()) {
                        message = new TextComponent("This Doberman demonstrates the recessive dilution trait and carries the red fur trait.");
                    } else if (this.carriesDilute()) {
                        message = new TextComponent("This Doberman carries two recessive traits.");
                    } else {
                        message = new TextComponent("This Doberman carries the red fur trait.");
                    }
                } else if (this.isDilute()) {
                    message = new TextComponent("This Doberman demonstrates the recessive dilution trait.");
                } else if (this.carriesDilute()) {
                    message = new TextComponent("This Doberman carries the dilution trait.");
                } else {
                    message = new TextComponent("This Doberman doesn't have any recessive traits.");
                }
