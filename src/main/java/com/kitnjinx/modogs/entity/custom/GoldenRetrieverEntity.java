...
public class GoldenRetrieverEntity extends AbstractDog {

    // handles coat variant
    private static final EntityDataAccessor<Integer> DATA_ID_TYPE_VARIANT =
            SynchedEntityData.defineId(GoldenRetrieverEntity.class, EntityDataSerializers.INT);
    ...
    public GoldenRetrieverEntity(EntityType<? extends TamableAnimal> entityType, Level level) {
    ...
    protected void registerGoals() {
        ...
        this.targetSelector.addGoal(7, new NonTameRandomTargetGoal<>(this, Zombie.class, true, PREY_SELECTOR));
        this.targetSelector.addGoal(7, new NonTameRandomTargetGoal<>(this, ZombieVillager.class, true, PREY_SELECTOR));
    }

    @Nullable
    @Override
    public AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob otherParent) {
        GoldenRetrieverEntity baby = ModEntityTypes.GOLDEN_RETRIEVER.get().create(serverLevel);

        determineBabyVariant(baby, (GoldenRetrieverEntity) otherParent);
    ...
    private <T extends GeoAnimatable> PlayState predicate(AnimationState<T> state) {
        if (this.isSitting()) {
            state.getController().setAnimation(RawAnimation.begin().then("animation.golden_retriever.sitting", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }

        if (this.isAngry() || this.isAggressive() && state.isMoving()) {
            state.getController().setAnimation(RawAnimation.begin().then("animation.golden_retriever.angrywalk", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }

        if (state.isMoving()) {
            state.getController().setAnimation(RawAnimation.begin().then("animation.golden_retriever.walk", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }

        if (this.isAngry() || this.isAggressive()) {
            state.getController().setAnimation(RawAnimation.begin().then("animation.golden_retriever.angryidle", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }

        state.getController().setAnimation(RawAnimation.begin().then("animation.golden_retriever.idle", Animation.LoopType.LOOP));
        return PlayState.CONTINUE;
    }
    @Override
        ...
        if (item == ModItems.GENE_TESTER.get()) {
            if (this.level().isClientSide) {
                ...
                if (this.getVariant() == ShadeVariant.LIGHT) {
                    message = Component.literal("This Golden Retriever has the alleles for light fur.");
                } else if (this.getVariant() == ShadeVariant.MEDIUM) {
                    message = Component.literal("This Golden Retriever has the alleles for medium fur.");
                } else {
                    message = Component.literal("This Golden Retriever has the alleles for dark fur.");
                }
