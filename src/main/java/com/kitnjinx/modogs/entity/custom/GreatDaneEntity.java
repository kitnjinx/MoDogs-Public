...
public class GreatDaneEntity extends AbstractDog {
    ...
    private <E extends GreatDaneEntity> PlayState predicate(AnimationEvent<E> event) {
        if (this.isSitting()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.great_dane.sitting"));
            return PlayState.CONTINUE;
        }

        if (this.isAngry() || this.isAggressive() & event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.great_dane.angrywalk"));
            return PlayState.CONTINUE;
        }

        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.great_dane.walk"));
            return PlayState.CONTINUE;
        }

        if (this.isAngry() || this.isAggressive()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.great_dane.angryidle"));
            return PlayState.CONTINUE;
        }

        event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.great_dane.idle"));
        return PlayState.CONTINUE;
    }
    @Override
        ...
        if (item == ModItems.GENE_TESTER.get()) {
            if (this.level.isClientSide) {
                ...
                if (this.getVariant() == GreatDaneVariant.FAWN) {
                    if (this.isBlue()) {
                        message = new TextComponent("This Great Dane demonstrates a recessive fur color and has the alleles for the recessive dilution trait.");
                    } else if (this.isBlueCarrier()) {
                        message = new TextComponent("This Great Dane demonstrates a recessive fur color and carries the recessive dilution trait.");
                    } else {
                        message = new TextComponent("This Great Dane demonstrates a recessive fur color.");
                    }
                } else if (this.isBlue()) {
                    if (this.isFawnCarrier()) {
                        message = new TextComponent("This Great Dane demonstrates the recessive dilution trait, and also carries recessive fawn fur.");
                    } else {
                        message = new TextComponent("This Great Dane demonstrates the recessive dilution trait.");
                    }
                } else {
                    if (this.isFawnCarrier() && this.isBlueCarrier()) {
                        message = new TextComponent("This Great Dane carries two recessive traits.");
                    } else if (this.isFawnCarrier()) {
                        message = new TextComponent("This Great Dane carries the recessive fawn fur trait.");
                    } else if (this.isBlueCarrier()) {
                        message = new TextComponent("This Great Dane carries the recessive dilution trait.");
                    } else {
                        message = new TextComponent("This Great Dane doesn't have any recessive traits.");
                    }
                }
