...
public class ItalianGreyhoundEntity extends AbstractDog {
    ...
    private <E extends ItalianGreyhoundEntity> PlayState predicate(AnimationEvent<E> event) {
        if (this.isSitting()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.italian_greyhound.sitting"));
            return PlayState.CONTINUE;
        }

        if (this.isAngry() || this.isAggressive() & event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.italian_greyhound.angrywalk"));
            return PlayState.CONTINUE;
        }

        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.italian_greyhound.walk"));
            return PlayState.CONTINUE;
        }

        if (this.isAngry() || this.isAggressive()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.italian_greyhound.angryidle"));
            return PlayState.CONTINUE;
        }

        event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.italian_greyhound.idle"));
        return PlayState.CONTINUE;
    }
    @Override
        ...
        if (item == ModItems.GENE_TESTER.get()) {
            if (this.level.isClientSide) {
                ...
                if (this.isBlue() && this.isFawn()) {
                    if (this.hasWhite()) {
                        message = new TextComponent("This Italian Greyhound demonstrates the recessive dilution trait and has white markings. They also have the alleles for fawn fur.");
                    } else {
                        message = new TextComponent("This Italian Greyhound demonstrates the recessive dilution trait. They also have the alleles for fawn fur.");
                    }
                } else if (this.isBlue()) {
                    if (this.hasWhite() && this.carriesFawn()) {
                        message = new TextComponent("This Italian Greyhound demonstrates the recessive dilution trait and has white markings. They also carry the fawn fur trait.");
                    } else if (this.hasWhite()) {
                        message = new TextComponent("This Italian Greyhound demonstrates the recessive dilution trait and has white markings.");
                    } else if (this.carriesFawn()) {
                        message = new TextComponent("This Italian Greyhound demonstrates the recessive dilution trait and carries the trait for fawn fur.");
                    } else {
                        message = new TextComponent("This Italian Greyhound demonstrates the recessive dilution trait.");
                    }
                } else if (this.isFawn()) {
                    if (this.hasWhite() && this.carriesBlue()) {
                        message = new TextComponent("This Italian Greyhound demonstrates the recessive fawn fur trait and has white markings. They also carry the dilution trait.");
                    } else if (this.hasWhite()) {
                        message = new TextComponent("This Italian Greyhound demonstrates the recessive fawn fur trait and has white markings.");
                    } else if (this.carriesBlue()) {
                        message = new TextComponent("This Italian Greyhound demonstrates the recessive fawn fur trait and carries the dilution trait.");
                    } else {
                        message = new TextComponent("This Italian Greyhound demonstrates the recessive fawn fur trait.");
                    }
                } else if (this.carriesBlue() && carriesFawn()) {
                    if (this.hasWhite()) {
                        message = new TextComponent("This Italian Greyhound demonstrates white markings and carries two recessive traits.");
                    } else {
                        message = new TextComponent("This Italian Greyhound carries two recessive traits.");
                    }
                } else if (this.carriesBlue()) {
                    if (this.hasWhite()) {
                        message = new TextComponent("This Italian Greyhound demonstrates white markings and carries the dilution trait.");
                    } else {
                        message = new TextComponent("This Italian Greyhound carries the dilution trait.");
                    }
                } else if (this.carriesFawn()) {
                    if (this.hasWhite()) {
                        message = new TextComponent("This Italian Greyhound demonstrates white markings and carries the fawn fur trait.");
                    } else {
                        message = new TextComponent("This Italian Greyhound carries the fawn fur trait.");
                    }
                } else {
                    if (this.hasWhite()) {
                        message = new TextComponent("This Italian Greyhound demonstrates white markings.");
                    } else {
                        message = new TextComponent("This Italian Greyhound doesn't have any recessive traits.");
                    }
                }
