...
public class GenoReaderItem extends Item {
    ...
    public InteractionResult interactLivingEntity(ItemStack stack, Player player, LivingEntity target, InteractionHand hand) {
        if (player.level().isClientSide && target instanceof AbstractDog) {
            ...
            } else if (target instanceof CollieEntity) {
                testResult = collieGeno((CollieEntity) target);
            ...
            } else if (target instanceof GermanSpitzEntity) {
                testResult = germanSpitzGeno((GermanSpitzEntity) target);
            ...
            Component output = Component.literal(beginning + testResult);
            LAST_GENO = output.getString();
            player.sendSystemMessage(output);
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        if (this.LAST_GENO != null && level.isClientSide) {
            player.sendSystemMessage(Component.literal(LAST_GENO));
        }
    ...
    private String collieGeno(CollieEntity dog) {
        if (dog.isBlack()) {
            if (dog.isMerle()) {
                return "bb - Merle Positive";
            } else {
    ...
    private String germanSpitzGeno(GermanSpitzEntity dog) {
        if (dog.getVariant() == GermanSpitzVariant.WHITE) {
            if (dog.isBrown()) {
                if (dog.isRed()) {
                    return "bb ee ww";
                } else if (dog.carriesRed()) {
    ...
    private String whippetGeno(WhippetEntity dog) {
        ...
        } else if (dog.isPureWhite() && dog.isBlueCarrier()) {
            if (dog.isRed()) {
                return "bb Dd WW";
            } else if (dog.isRedCarrier()) {
                return "Bb Dd WW";
